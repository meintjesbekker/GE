...
void CDecimateSmoothNormals::Decimate(vtkPolyData* pcPolyData, BOOL bDecimate, float fTargetReduction)
{
	if (bDecimate)
	{
		...
		m_pcDecimatePro->SetInput(pcPolyData);
...
void CDecimateSmoothNormals::SmoothPolyData(vtkPolyData* pcPolyData, BOOL bSmooth, BOOL bDecimate, int iNumberOfIterations, float fRelaxationFactor)
{
	if (bSmooth)
	{
		...
		if (bDecimate)
			m_pcSmoothPolyDataFilter->SetInput(m_pcDecimatePro->GetOutput());
		else
			m_pcSmoothPolyDataFilter->SetInput(pcPolyData);
...
void CDecimateSmoothNormals::ComputePolyDataNormals(vtkPolyData* pcPolyData, vtkPolyData* pcCleanPolyData, BOOL bSmooth, BOOL bDecimate, BOOL bAverage)
{
	...
	if (bSmooth)
		m_pcPolyDataNormals->SetInput(m_pcSmoothPolyDataFilter->GetOutput());
	else 
		if (bDecimate)
			m_pcPolyDataNormals->SetInput(m_pcDecimatePro->GetOutput());
		else 
			if (bAverage)
				m_pcPolyDataNormals->SetInput(pcPolyData);
			else
				m_pcPolyDataNormals->SetInput(pcCleanPolyData);
}